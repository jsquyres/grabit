...
func TestNewResourceFromUrl(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			valid: true,
			res:   Resource{Urls: []string{fmt.Sprintf("http://localhost:%d/test.html", port)}, Integrity: fmt.Sprintf("%s-vvV+x/U6bUC+tkCngKY5yDvCmsipgW8fxsXG3Nk8RyE=", algo), Tags: []string{}, Filename: "", CacheUri: "cacheUri"},
		},
		{
	...
	for _, data := range tests {
		resource, err := NewResourceFromUrl(data.urls, algo, []string{}, "", "cacheUri")
